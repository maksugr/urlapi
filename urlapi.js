...
export function serialize(url) {

    //check on object and fields exist
    if (typeof url !== 'object' || Array.isArray(url) || !checkObj(url)) {
    ...
    function protocol (url) {
        if (url.protocol !== undefined && typeof url.protocol === 'string') {
            ...
            let protocol = url.protocol.replace(/\W/g, '').trim();
    ...
    function auth (url) {
        ...
        } else if (url.auth !== undefined && url.auth !== null && typeof url.auth === 'object' && !Array.isArray(url.auth)) {
    ...
    function searchAndQuery(url) {
        ...
        //else use query
            ...
            } else if (url.query !== undefined && typeof url.query === 'object' && !Array.isArray(url.query)) {
    ...
    function hash(url) {
        ...
        } else if (url.hash !== undefined && typeof url.hash === 'object' && !Array.isArray(url.hash)) {
            //add #
            resultStringUrl += '#';
...
export function parse(url) {
    ...
    function auth(url) {
        ...
        if (url.indexOf('@') > -1) {
            if (resultObj.protocol !== null) {
                ...
                let authHolder = resultObj.auth.split(':');
                resultObj.auth = {
                    login: authHolder[0],
    ...
    function hostnamePort() {
        if (resultObj.host !== null) {
            ...
            let devideOnTwoParts = resultObj.host.split(':');
